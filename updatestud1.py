...
lab = tk.Label(root, font= ( 'aria' ,30, 'bold' ),bg="lightpink",text="Update Student email-id",fg="steel blue",bd=10,anchor='w')
...
lab2=tk.Label(root,text="email-id",bg="lightpink", fg="blue",font="Consolas 20 bold")
...
def updateDetails():
    ...
    query="update student_details set email_id='%s' where srn='%s'"%(e2.get(),e1.get())
    cur.execute(query)
    ...
    if row>0:
        op.config(text="Record Updated",bg="lightpink", fg="green") 
    else:
       op.config(text="Record Not Updated",bg="lightpink", fg="red")
...
b1.grid(row=10, column=1,pady=30)

op=tk.Label(root,bg="lightpink",font="Consolas 20 bold")
op.grid(row=8, column=0, columnspan=2)
